...
void drawSnowman(struct Drawing *d, struct Point centerBottom,double height)
{
  ...
  initCircle(&bottom, makePoint(centerBottom.x, centerBottom.y - 2.0 * unit),2.0 * unit);

  initCircle(&middle, makePoint(centerBottom.x, centerBottom.y - 5.5 * unit),1.5 * unit);

  initCircle(&top, makePoint(centerBottom.x, centerBottom.y - 8.0 * unit),1.0 * unit);
